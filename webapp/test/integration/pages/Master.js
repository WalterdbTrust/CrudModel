			...
			actions: {
									...
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function(oObject) {
											return !oList.getItems().some(function(oListItem) {
												return oListItem.getBindingContext().getProperty("MODEL_ID") === oObject.MODEL_ID;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].MODEL_ID;
									} else {
										sCurrentId = aItemsNotInTheList[0].MODEL_ID;
									}
				...
				iRememberTheListItem: function(oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("MODEL_ID"),
						title: oBindingContext.getProperty("MODEL_ID")
					};
				},
			...
			assertions: {
				...
				theListHeaderDisplaysZeroHits: function() {
					return this.waitFor({
						...
						success: function() {
							Opa5.assert.ok(true, "The list header displays 'P_MODEL (0)'");
						},
				...
				theListShouldBeSortedAscendingOnName: function() {
					return this.theListShouldBeSortedAscendingOnField("MODEL_ID");
				},
				...
				theListShouldHaveAllEntries: function() {
					...
					// retrieve all P_MODEL to be able to check for the total amount
