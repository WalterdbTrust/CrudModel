var pathModel = "";
sap.ui.define([
	"ModelCrud/controller/BaseController",
	...
	return BaseController.extend("ModelCrud.controller.CreateEntity", {
		...
		_onEdit: function(oEvent) {
			...
			oView.bindElement({
				...
				pathModel = "/P_MAKE";
			var mySelectMenuMODEL = sap.ui.getCore().byId("mySelectMenuPurpose");
			...
			mySelectMenuMODEL.bindAggregation("items", {
				...
				mySelectMenuMODEL.setSelectedKey(sap.ui.getCore().byId("inputPurpose").getValue());
				});
		},
		...
		_onCreate: function(oEvent) {
			...
			var oContext = this._oODataModel.createEntry("P_MODEL", {
				success: this._fnEntityCreated.bind(this),
				error: this._fnEntityCreationFailed.bind(this)
			});
			this.getView().setBindingContext(oContext);
				var mySelectMenuMAKEP = sap.ui.getCore().byId("mySelectMenuPurpose");
		console.log(mySelectMenuMAKEP)
			...
			mySelectMenuMAKEP.bindAggregation("items", {
				path: pathModel,
				sorter:new sap.ui.model.Sorter({path:"DESCRIPTION",descending:false}),
				template: oItemSelectTemplate
			});
				mySelectMenuMAKEP.getBinding("items").attachDataReceived(function() {
				var marrayOri = mySelectMenuMAKEP.getBinding("items").getContexts();
					console.log(marrayOri)
			var a=mySelectMenuMAKEP.mProperties.selecteKey;
				console.log(a)
				debugger
			// 	if(a){
			// 			sap.ui.getCore().byId("inputPurpose").setValue(a);
			// 	}else{
			// 		sap.ui.getCore().byId("inputPurpose").setValue(marrayOri[0].getObject().MAKE_ID);
			// 	}
				// var marrayLength = mySelectMenuMODEL.getBinding().getLength();
				mySelectMenuMAKEP.setSelectedKey(marrayOri[0].getObject().MAKE_ID)
				sap.ui.getCore().byId("inputPurpose").setValue(marrayOri[0].getObject().MAKE_ID);
				});
		},
		...
		_fnEntityCreated: function(oData) {
			var sObjectPath = this.getModel().createKey("P_MODEL", oData);
