...
sap.ui.define([
	...
	return BaseController.extend("ModelCrud.controller.Master", {
		...
		onSearch: function(oEvent) {
			...
			if (sQuery) {
				this._oListFilterState.aSearch = [new Filter("MODEL_ID", FilterOperator.Contains, sQuery)];
			} else {
				...
				function(mParams) {
					...
					if (!mParams.list.getSelectedItem()) {
						this.getRouter().navTo("object", {
							MODEL_ID: encodeURIComponent(mParams.firstListitem.getBindingContext().getProperty("MODEL_ID"))
						}, true);
					}
				}.bind(this),
				function(mParams) {
		...
		_showDetail: function(oItem) {
			...
			this.getRouter().navTo("object", {
				MODEL_ID: encodeURIComponent(oItem.getBindingContext().getProperty("MODEL_ID"))
			}, bReplace);
		},
